...
public class PlayerGun : Gun {
	...
	protected virtual void ApplyMovement(float time)
	{
		...
		if (verticalOffset.min < verticalOffset.max)
		{
			// If gun's position.y is out of restrict 
			if (transform.position.y > VerticalPositionMax().y)
			{
				// Then change verticalOperator
				verticalOperator = -verticalOperator;

				// Set position to max vertical, avoid over-value
				transform.position = VerticalPositionMax();
			}
			else if (transform.position.y < VerticalPositionMin().y)
			{
				// Then change verticalOperator
				verticalOperator = -verticalOperator;

				// Set position to min vertical, avoid over-value
				transform.position = VerticalPositionMin();
			}
	...
	// Return vertical max position
	protected virtual Vector3 VerticalPositionMax()
	{
		return new Vector3(OffsetPosition().x, OffsetPosition().y + verticalOffset.max);
	}


	// Return vertical min position
	protected virtual Vector3 VerticalPositionMin()
	{
		return new Vector3(OffsetPosition().x, OffsetPosition().y + verticalOffset.min);
	}
